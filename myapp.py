...
PROJECT_ID = 12  # Assumes the project already exists in WebODM
...
@app.route('/check_webodm/<task_id>', methods=['GET'])
def check_webodm(task_id):
    info = task_status_store.get(task_id)
    if not info:
...
def volume_pipeline(task_id, before_paths, after_paths):
    ...
    if volume is not None:
        task_status_store[task_id] = {"status": "completed", "volume_change": round(volume, 2)}
    else:
        task_status_store[task_id] = {"status": "failed"}
...
def compute_volume(before_file, after_file):
    cloudcompare_path = "CloudCompare"  # Or full path if needed
    ...
    try:
        ...
        logging.info("CloudCompare STDOUT:\n" + result.stdout)
        logging.info("CloudCompare STDERR:\n" + result.stderr)
        ...
        logging.error("CloudCompare execution failed:")
